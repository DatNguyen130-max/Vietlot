...
import { loadGithubSnapshot, loadLocalSnapshot, type SnapshotPayload } from "@/lib/snapshot";
...
type SyncSourceType = "local" | "github";
...
function parseSyncSource(sourceValue: string | null): SyncSourceType {
  ...
  if (normalized === "github" || normalized === "remote" || normalized === "raw") {
    return "github";
  }
  throw new Error("Invalid source. Use source=local or source=github.");
}

async function loadSnapshotForSource(game: GameType, sourceType: SyncSourceType): Promise<SnapshotPayload> {
  if (sourceType === "github") {
    return loadGithubSnapshot(game);
  }

  return loadLocalSnapshot(game);
}
...
async function performSync(request: NextRequest): Promise<NextResponse> {
  ...
  for (const game of games) {
    ...
    const sourceResult = await loadSnapshotForSource(game, sourceType);
